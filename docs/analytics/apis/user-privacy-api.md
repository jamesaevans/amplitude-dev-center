...
- By default, deletion requests only create jobs for a particular project within your organization. This project is identified by the api key provided in the request. If you would like to delete a user across your entire organization, set both the `delete_from_org` and `ignore_invalid_ids` parameters to `true` in your request. The API will then create one deletion job per project that contains information on the requested users. 
...
| `ignore_invalid_id`                | Boolean. Defaults to `false`, which will return a 400 error if any of the provided users are not found in the project(s) requested. With `false`, no users in the request will be added to a deletion job. If `true`, the response will be a 200 success that includes a list of `invalid_ids`, and any users found in the project(s) will be added to the relevant job(s). We recommend setting this to `true` if setting the `delete_from_org` parameter to `true`                                                                                                                                                                                                                    |
| `delete_from_org`                  | Boolean. Defults to `false`. When `false`, it will only delete the requested users from the project identified by the givene api key. When `true`, it will delete the users from the entire org (all projects) instead of a single project. It will insert the requested users into a job for each project that contains data for the requested users. When `true`, we recommend also setting the `ignore_invalid_ids` parameter to `true`.                                                                                                                          |
