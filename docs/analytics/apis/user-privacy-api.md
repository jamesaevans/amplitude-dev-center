...
| `ignore_invalid_id`                | Boolean. Defaults to `false`, and returns a 400 error if any of the provided users are not found in the project(s) requested. With `false`, no users in the request are  added to a deletion job. If `true`, the response is a 200 success that includes a list of `invalid_ids`, and any users found in the project(s) are added to the relevant job(s). Amplitude recommends setting this to `true` if setting the `delete_from_org` parameter to `true`                                                                                                                                                                                                                    |
